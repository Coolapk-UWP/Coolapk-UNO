...
namespace CoolapkUNO.Helpers
{
    public static class RequestHelper
    {
        ...
        public static async Task<(bool isSucceed, JsonElement result)> GetDataAsync(Uri uri, bool isBackground = false)
        {
            if (uri == null) { return (false, default); }
            DataContainer<JsonElement> results = await NetworkHelper.GetAsync(uri, GetCoolapkCookies(uri), SourceGenerationContext.Default.DataContainerJsonElement, NetworkHelper.XMLHttpRequest, isBackground).ConfigureAwait(false);
            if (results == null) { return (false, default); }
            if (!string.IsNullOrEmpty(results.Message))
            {
                _ = UIHelper.ShowMessageAsync(results.Message);
